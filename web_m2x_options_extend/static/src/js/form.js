            ...
            if (_.isUndefined(this.ir_options_loaded)) {
                ...
                web_m2x_options.then(function (records) {
                    _(records).each(function (record) {
                        ...
                        if (record.key == 'web_m2x_options.create_false_effect') {
                            // var effect_list = record.value.split(',')
                            if (self.check_model_in_value(self.field.relation, record.value) != -1) {
                                // console.log(self.field.relation)
                                self.effect_flag = true;
                                self.can_create = false;
                            }
                        }
                        if (record.key == 'web_m2x_options.can_open_no_write') {
                            if (self.check_model_in_value(self.field.relation, record.value) != -1) {
                    ...
                    if (!self.effect_flag && self.ir_options.hasOwnProperty('web_m2x_options.create_false_effect')) {
                        self.ir_options = {}
                    }
                    ;
                ...
                // console.log(self.ir_options)
            }

            return $.when();
        },

        check_model_in_value: function (model, value) {
